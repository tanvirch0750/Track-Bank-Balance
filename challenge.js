...
const checkDogs = (dogsJulia, dogsKate) => {
  let newDogsJulia = dogsJulia.slice(0, -2);
...
Test data: § Data 1: [5, 2, 4, 1, 15, 8, 3] § Data 2: [16, 6, 10, 5, 6, 1, 4]  
GOOD LUCK 😀  
*/

const calcAverageHumanAge = ages => {
  const adultDogAges = ages
    .map(dogAge => {
      if (dogAge <= 2) {
        return 2 * dogAge;
      } else {
    ...
    .filter(adAge => adAge >= 18);

  const dogAgesSum = adultDogAges.reduce((acc, age) => acc + age, 0);

  const dogAverage = dogAgesSum / adultDogAges.length;
